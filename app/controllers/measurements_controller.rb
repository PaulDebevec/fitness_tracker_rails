...
class MeasurementsController < ApplicationController
  ...
  before_action -> { require_profile_owner_or_admin(@profile) }, only: %i[create edit update destroy]
  ...
  def update
    if @measurement.update(measurement_params)
      redirect_to profile_check_in_measurement_path(@profile, @check_in, @measurement),
